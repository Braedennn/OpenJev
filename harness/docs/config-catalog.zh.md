...
export interface Config {
  /** OpenJev home directory exposed as `DSH_HOME`; defaults to `$DSH_HOME` or `~/.dsh`. */
  dshHome?: string
}
```
...
export interface Config {
  ...
  /** OpenJev config root. Defaults to `$DSH_HOME` or `~/.dsh`. */
  dshHome?: string
...
export interface Config {
  /** Include the fixed OpenJev identity before the deployment persona (default true). */
