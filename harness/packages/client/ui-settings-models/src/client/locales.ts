...
export const en = {
  ...
  welcomeBody: "OpenJev 0.1 remains in testing for OpenJev developers. Many areas need further improvement, and we welcome feedback from the developer community. OpenJev's core plugins and foundational APIs will continue to evolve rapidly over the coming months.\n\nWe look forward to exploring the limits of intelligence with developers around the world, building on open-source, open, reusable, and composable infrastructure. We welcome OpenJev developers everywhere to join the OpenJev plugin ecosystem.",
  welcomeContinue: 'Continue',
  welcomeError: 'The acknowledgement could not be saved. Please try again.',
  onboardingTitle: 'Add an API key to get started',
  onboardingDescription: 'Pick any provider — DeepSeek, OpenAI, Anthropic, OpenRouter, a local server, or any OpenAI-compatible gateway — and add its API key. Every provider row below works the same way.',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  creating: '创建中\u2026',
  welcomeTitle: '内测声明',
  welcomeBody: 'OpenJev 目前的 0.1 版本仍处在面向 OpenJev 开发者进行测试的阶段，还有许多地方需要持续改进和打磨，希望听取广大开发者的反馈建议。预计 OpenJev 的核心插件以及基础 API 都会在接下来的一段时间内快速迭代、持续演化。\n\n我们期待与全球开发者一起，在开源、开放、可复用、可组合的基础设施之上，共同探索智能上限。欢迎全球 OpenJev 开发者加入 OpenJev 插件生态。',
  welcomeContinue: '继续',
  welcomeError: '暂时无法保存确认状态，请重试。',
  onboardingTitle: '添加一个 API Key 开始使用',
  onboardingDescription: '任选一个提供方——DeepSeek、OpenAI、Anthropic、OpenRouter、本地服务，或任何兼容 OpenAI 的网关——填入其 API 密钥即可。下方每个提供方的用法都相同。',
